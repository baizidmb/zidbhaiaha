...
export default function ParrondosParadox({ lang = 'bn' }) {
  ...
  const chartOptions = {
    ...
    scales: {
      x: { title: { display: true, text: isBn ? 'কয়েন টস স্টেপ' : 'Flips', color: '#64748b' } },
      y: { title: { display: true, text: isBn ? 'মূলধন ($)' : 'Capital ($)', color: '#64748b' } }
    }
  };
      ...
      <div className="glass-card rounded-3xl p-4 sm:p-8 border border-slate-800 shadow-2xl relative overflow-hidden space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center space-x-3">
              ...
              <h2 className="text-xl sm:text-3xl font-extrabold text-slate-100 tracking-tight">
                {isBn ? '🪙 জেতা-হারার ট্রিক (The Winning-Loss Trick)' : '🪙 The Winning-Loss Trick'}
              </h2>
            </div>
            <p className="text-slate-400 text-xs sm:text-sm mt-1 font-medium">
              {isBn ? 'দুটি আলাদাভাবে হারা গেম অল্টারনেট করে খেললে একটি বিজয়ী ট্রেন্ড তৈরি হয়!' : 'Combine two individually LOSING games to produce a steadily WINNING trend!'}
            </p>
          </div>
          ...
          >
            ...
            <span>{isSimulating ? `${simProgress}%` : (isBn ? 'নতুন ফ্লিপ ট্রায়াল' : 'Run Flip Trial')}</span>
          </button>
        </div>

        {/* Step 1: 2-Sentence ELI5 Story Banner BEFORE Charts */}
        <div className="p-4 rounded-2xl bg-gradient-to-r from-emerald-950/60 via-slate-900 to-teal-950/60 border border-emerald-500/30 space-y-2">
          ...
          <p className="text-xs sm:text-sm text-slate-200 leading-relaxed font-medium">
            {isBn 
              ? 'দুই বন্ধু দুটি পৃথক কয়েন গেম তৈরি করল যেখানে আলাদাভাবে খেললে টাকা শুধুই কমতে থাকে। কিন্তু অল্টারনেট বা বিকল্প হিসেবে খেললে গেম A গেম B-এর খারাপ ফাঁদটি ভেঙে দেয় এবং ব্যাংক ব্যালেন্স রকেটের মত লাফিয়ে বাড়ে!' 
              : 'Can playing two LOSING games result in a WINNING strategy? Parrondo\'s Paradox proves that alternating between two bad coin games breaks the trap cycle and creates a ratcheting upward profit!'}
          </p>
        </div>

        {/* Step 2: Simplified Piggy Bank Capital Readouts */}
        ...
        {/* Step 3: Collapsible Multi-Line Trajectory Chart Drawer */}
        <details className="group border border-slate-800/80 rounded-2xl bg-slate-900/60 overflow-hidden">
          <summary className="px-5 py-4 flex items-center justify-between cursor-pointer font-bold text-sm text-slate-200 hover:bg-slate-800/60 transition-colors">
            <span className="flex items-center space-x-2">
              ...
              <span>{isBn ? 'কয়েন টস ট্র্যাজেক্টোরি গ্রাফ ও স্টেপস দেখুন (Deep Dive Graph)' : 'View Coin Flip Trajectory Graph & Step Tuner'}</span>
            </span>
          ...
          <div className="p-5 border-t border-slate-800 space-y-4">
            <div className="flex justify-between items-center bg-slate-950 p-3 rounded-xl border border-slate-800">
              <span className="text-xs font-bold text-slate-300">
                {isBn ? 'কয়েন টস স্টেপ:' : 'Steps:'} <strong className="text-emerald-400">{totalSteps}</strong>
              </span>
              <input
                type="range"
                min="50"
        ...
        {/* Step 4: Explainer Drawer */}
        <ExplainerDrawer
          title={isBn ? "জেতা-হারার ট্রিক (The Winning-Loss Trick)" : "The Winning-Loss Trick"}
          eli5={isBn 
            ? "এমন দুটি কয়েন গেমের কথা ভাবুন যেখানে আলাদাভাবে খেলতে গেলে আপনার টাকা কমতেই থাকে। কিন্তু দুটি গেম পর্যায়ক্রমে বদলে বদলে খেললে গেম A প্লেয়ারকে খারাপ ফাঁদ থেকে বের করে আনে এবং টাকা বহুগুণে বাড়তে শুরু করে!"
            : "Imagine two games where you lose money over time. But if you alternate between Game A and Game B, Game A breaks the bad trap cycle of Game B, causing your money to shoot UP!"}
          intuitionTrap={isBn 
            ? "আমাদের স্বজ্ঞাত ধারণা বলে দুটি ঋণাত্মক ফলাফল যোগ করলে ঋণাত্মকই হয় (-২ + -২ = -৪)। কিন্তু এখানে পর্যায়ক্রমিক পরিবর্তন ফলাফলের মোমেন্টাম বদলে দেয়।"
            : "Common sense dictates that adding two negative numbers gives a negative number (-2 + -2 = -4). So combining two losing games SHOULD yield a super-losing game! We fail to realize that Game A breaks the bad cycle trap of Game B."}
...
Alternating A+B yields positive expected value (+0.016 EV)!`}
          realWorld={isBn 
            ? "বায়োলজি ও পোর্টফোলিও রিব্যালেন্সিং (Volatility Harvesting)।"
            : "Biological Survival & Portfolio Rebalancing (Volatility Harvesting)."}
        />
